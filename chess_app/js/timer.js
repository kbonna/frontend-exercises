    ...
    tickTimer() {
        ...
        if (!this.paused) {
            if (this.game.whoseTurn === 1) {
                ...
                this.clockBlack.innerHTML = Timer.getPrintableTime(Math.round(this.maxTime - this.tElapsedBlack / 1000));
            } else {
                tElapsed = (Date.now() - this.initTime + this.tElapsedBlack) / 1000;
                tLeft = Math.round(this.maxTime - tElapsed);
                this.clockBlack.innerHTML = Timer.getPrintableTime(tLeft);
                this.clockWhite.innerHTML = Timer.getPrintableTime(Math.round(this.maxTime - this.tElapsedWhite / 1000));
            }
